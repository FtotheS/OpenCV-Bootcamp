...
import os
import dlib
from scipy.special import distance as dist
...
def verify_eye_blink(self):
    try:
        blink_count_list = []
        t_end = time.time() + 5   # wait 5 seconds
        ...
        while time.time() - t_end:
            ...
            for face in faces:
                shape = self.landmar_predict(img_gray, face)
                ...
                img = self.mark_eyeLandmar(img, [lefteye, righteye])
                avg = (left_EAR + right_EAR) / 2

                if avg < self.blink_thresh:
                    self.count_frame += 1
                elif self.count_frame >= self.succ_frame:
                    cv2.putText(img, "Blink detected", (30,30), cv2.FONT_HERSHEY_DEPLEX, 1, (0,255,0), 1)
                    blink_count_list.append("Blink detected")
                    self.count_frame = 0
                else:
                    cv2.putText(img, "Blink your eyes !!!", (30,30), cv2.FONT_HERSHEY_DEPLEX, 1, (0,0,255), 1)

            cv2.imgshow("Eye blink verify", img)
